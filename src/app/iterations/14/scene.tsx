...
export default function Scene({ images }: { images: SceneImage[] }) {
  ...
  useEffect(() => {
    ...
    // Centered title, drawn to a canvas and mapped onto a plane so images can
    // layer in front of or behind it inside the 3D scene. It lives at TEXT_Z=0;
    // images get a random +z (in front) or -z (behind).
    const TEXT_Z = 0;
    const TITLE_LINES = ["Karl", "Monies"];
    const TITLE_PX = 150; // css-like font size per line
    const SS = 4; // supersample factor for a crisp texture
    const titleFont = `700 ${TITLE_PX * SS}px "Helvetica Neue", Helvetica, Arial, sans-serif`;
    const titleCanvas = document.createElement("canvas");
    const tctx = titleCanvas.getContext("2d")!;
    tctx.font = titleFont;
    const lineH = TITLE_PX * SS * 1.0; // lineHeight: 1
    const textW = Math.max(...TITLE_LINES.map((l) => tctx.measureText(l).width));
    titleCanvas.width = Math.max(1, Math.ceil(textW));
    titleCanvas.height = Math.max(1, Math.ceil(lineH * TITLE_LINES.length));
    tctx.font = titleFont; // resizing the canvas resets the context
    tctx.textAlign = "center";
    tctx.textBaseline = "middle";
    tctx.fillStyle = "#000000";
    TITLE_LINES.forEach((l, i) =>
      tctx.fillText(l, titleCanvas.width / 2, lineH * (i + 0.5)),
    );
    ...
    scene.add(titleMesh);
    // Keep the title at ~TITLE_PX regardless of viewport size: the visible width
    // at z=0 is exactly WALL_WIDTH, so one css pixel is WALL_WIDTH/clientWidth
    // world units.
    const sizeTitle = () => {
      const worldPerPx = WALL_WIDTH / mount.clientWidth;
      titleMesh.scale.set(
        (titleCanvas.width / SS) * worldPerPx,
        (titleCanvas.height / SS) * worldPerPx,
        1,
      );
    };
    sizeTitle();
    ...
    const labelFont = `400 ${LABEL_PX * SS}px "Helvetica Neue", Helvetica, Arial, sans-serif`;
    ...
    const spawnLayout = async () => {
      ...
      // Learn each image's true (EXIF-corrected) aspect before laying out.
      const aspects = await Promise.all(picked.map((img) => loadAspect(img.url)));
      if (cancelled) return;
  ...
  // The title now lives inside the three.js scene (as a canvas-texture plane)
  // so images can layer in front of and behind it, so no DOM overlay here.
