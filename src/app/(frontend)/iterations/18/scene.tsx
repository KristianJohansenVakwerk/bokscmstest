...
// Loading: textures are preloaded in the background at low concurrency so their
// GPU uploads are spread out instead of hitching the animation, and a layout
// only ever uses images that are already loaded (see pickImages). This keeps
// every 2s swap instant and smooth.
const PRELOAD_POOL = 80; // cap how many images we ever preload for the intro
const PRELOAD_CONCURRENCY = 4; // simultaneous texture loads
...
export default function Scene({
  ...
  useEffect(() => {
    ...
    // A url is "ready" once its texture is decoded and its true aspect is known.
    ...
    // Background preloader. Loads a bounded, shuffled subset of the images at low
    // concurrency, caching each texture and its TRUE (EXIF-corrected) aspect —
    // the served width/height, since the optimizer auto-rotates rotated photos.
    // Layouts draw only from what this has finished, so swaps never wait on the
    // network and GPU uploads are spread out instead of bursting on each swap.
    const preloadOrder = shuffled(images.map((i) => i.url)).slice(
      0,
      PRELOAD_POOL,
    );
    let preloadIdx = 0;
    ...
    const pumpPreload = () => {
      while (
        !cancelled &&
        preloadActive < PRELOAD_CONCURRENCY &&
        preloadIdx < preloadOrder.length
      ) {
        const url = preloadOrder[preloadIdx++];
        preloadActive++;
        loader.load(
          textureUrl(url),
          (texture) => {
            ...
            aspectCache.set(url, img.width / img.height || 1);
            texCache.set(url, texture);
            pumpPreload();
          },
          undefined,
    ...
    // Pick COUNT distinct random images from the images that are ALREADY loaded,
    // so a layout never has to wait on the network mid-animation.
    const pickImages = (): SceneImage[] => {
      const ready = images.filter((im) => aspectCache.has(im.url));
      if (ready.length <= COUNT) return ready.slice();
    ...
    const spawn = (p: Placement) => {
      ...
      scene.add(mesh);
      applyTexture(p.img.url, material); // cache hit (preloaded) — no network
    ...
    // Build a fresh layout from already-loaded images. Synchronous — everything
    // it needs (aspect + texture) is cached by the preloader, so no awaiting.
    const spawnLayout = () => {
      const picked = pickImages();
      if (picked.length === 0) return;
      const sized = picked.map((img) => ({
        img,
        aspect: aspectCache.get(img.url) ?? 1,
      }));
    ...
    // Start the intro once the label font is ready and at least a few images have
    // preloaded, so the first layout appears fully-loaded rather than popping in.
    let auto: ReturnType<typeof setInterval> | undefined;
    const startIntro = () => {
      if (cancelled) return;
      if (aspectCache.size === 0) {
        window.setTimeout(startIntro, 100);
        return;
      }
      spawnLayout();
      auto = setInterval(swap, 2000);
    };
