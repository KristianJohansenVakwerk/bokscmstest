  ...
  return (
    <div className="relative flex flex-1 flex-col bg-zinc-50 font-sans">
      <main className="flex w-full flex-1 flex-col px-5 pt-5">
        {slides.length > 0 ? (
          <ul ref={gridRef} className="flex flex-wrap items-start gap-x-8 gap-y-4">
            {slides.map((post) => {
              ...
              return (
                <li
                  ...
                  ref={setTileRef}
                  data-id={id}
                  className="inline-flex cursor-pointer items-start gap-[15px] bg-[rgb(242,242,247)]"
                  onMouseEnter={(e) =>
                    setPreview(previewAt(e.clientX, url, alt))
                  }
                  onMouseLeave={() => setPreview(null)}
                >
                  {/* Caption first, always visible, in the regular font. */}
                  <span
                  ...
                  {/* Small thumbnail, lazy-loaded once the pair nears the viewport. */}
                  <span
