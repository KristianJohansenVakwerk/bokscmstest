...
export default function Wall({ images }: { images: WallImage[] }) {
  ...
  useEffect(() => {
    ...
    type Tile = { img: WallImage; material: THREE.MeshBasicMaterial };
    const tiles: Tile[] = [];
    ...
    // Mouse only nudges where the camera aims (a small parallax glance); the
    // wheel scrolls the camera vertically through the wall. The look-at point
    // tracks the camera's own height plus the mouse nudge, so scrolling moves
    // you up/down the grid while the glance stays relative.
    const pan = new THREE.Vector3(0, 0, 0);
    const desiredPan = new THREE.Vector3(0, 0, 0);
    const MAX_PAN = 1.4; // how far the glance can slide from center

    const onPointerMove = (e: PointerEvent) => {
      const rect = mount.getBoundingClientRect();
      const nx = ((e.clientX - rect.left) / rect.width) * 2 - 1; // -1..1
      const ny = ((e.clientY - rect.top) / rect.height) * 2 - 1;
      desiredPan.x = nx * MAX_PAN;
      desiredPan.y = -ny * MAX_PAN;
    };
    ...
    let raf = 0;
    const animate = () => {
      pan.lerp(desiredPan, 0.06);
      camera.position.y += (scrollTargetY - camera.position.y) * 0.12;
      target.set(pan.x, camera.position.y + pan.y, 0);
